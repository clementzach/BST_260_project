...
players <- read.csv('/Volumes/LACIE_SHARE/football_data/player_data.csv') #~500 MB file, so not in github
...
injuries_join_players <- left_join(x = injuries_df, 
                               y = players_small, 
                               by = c("year", "name", "team"))
...
sum(is.na(injuries_join_players$height_inches)) #180 didn't get joined. That's ok.

write.csv(injuries_join_players, paste("../injuries_players", Sys.Date() ,".csv", sep = ''))
