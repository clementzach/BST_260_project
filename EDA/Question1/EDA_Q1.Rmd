...
#write.csv(injuries, "injury_dat_q1.csv", row.names = F)
#write.csv(players, "players_dat_q1.csv", row.names = F)

```

&nbsp;

### Load data from .csv files saved to EDA Question1 folder
```{r}
injuries = read.csv("injury_dat_q1.csv", stringsAsFactors = T) %>% mutate(injury = factor(injury))
players = read.csv("players_dat_q1.csv", stringsAsFactors = T) %>% mutate(injury = factor(injury))
```
...
### avg. # of injuries per position per season
```{r}
# (meaning if I am a wide receiver, how many injuries (by type) can I expect to get this season?)
...
exp_injuries_byYear %>%
  ...
  ylim(1, 2.4) +
  xlab("Year") +
  ylab("Avgerage Total Injuries") +
  scale_color_discrete(name = "Position")

# can also do this for each injury type!!
...
logi_fit = glm(data = train_set, family = binomial(),
               injury ~ position_id + weight_pounds + game_starter + age + year)
...
```{r kNN}
knn_fit = knn3(data = train_set, injury ~ position_id + age + game_starter + 
                 weight_pounds + year, 
