...
```{r}
players = read.csv("all_players_some_injuries2021-11-15.csv")
head(players)

players = players %>% mutate(injury = ifelse(is.na(injury_types), 1, 0),
                             position = as.factor(position))



logi_fit = glm(data = players, injury ~ position + height + weight, family = binomial())
