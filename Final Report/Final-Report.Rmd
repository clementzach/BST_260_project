...
author: "Willow Duffell, Lauren Mock, Addison McGhee, Zach Clement, Dan Nolte"
date: "11/23/2021"
output: html_document
---

#### Instructions where to access the data

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

All of the data can be accessed inside the `Data` folder in the BST_260_project repository, available [here](https://github.com/clementzach/BST_260_project). 

- Once inside this folder, you will see the main 2 csv files that we used for analysis: `all_injuries.csv` and `all_player_demographic_clean.csv`. 
- Inside the data_build folder you will find functions used to webscrape the original data into the usable csv files. 
- The data dictionary folder includes a text file with variable descriptions. 
- The data_for_plots folder contains data sets used to create our EDA and analysis plots. 
  - These datasets are merged and modified versions of the two primary datasets (we saved these to avoid including a lot of cleaning code directly in our final Rmd file). The work code to clean and generate them is available in the `EDA` folder.

Our data was scraped from webpages such as [this one](https://www.pro-football-reference.com/teams/buf/2021_injuries.htm).

Additional demographic data was provided by the folks at gridironai.com, and can be downloaded [here](https://drive.google.com/drive/u/0/folders/1KTcoZRtcylZRc9rZk9zfA4PwJon-xtwB).
...
In order to avoid overloading the servers at [pro-football-reference.com](pro-football-reference.com), we paused our webscraping script for 5 seconds between accessing each webpage. 
...
Additional player demographic data (such as height, weight, age, and position) were provided by [gridironai.com](gridironai.com). Because this dataset had one row for every player for every week, duplicates were removed so that there was only one row per player per season. 

Although the dataset from [gridironai.com](gridironai.com) was the most complete of any dataset we identified on the internet (including attempting to webscrape aplayer information directly from [pro-football-reference.com](pro-football-reference.com)), it did not contain every player who had been listed as injured in the injury tables. We kept this in mind by carefully considering which types of join of the two datasets would result in selection bias for our analyses. 
...
data.frame(Model = c("Logistic", "kNN (k = 21)", "Random Forest"),
                     ...
                     Sensitivity = c(0.675, 0.619, 0.632),
                     Specificity = c(0.600, 0.539, 0.640),
                     AUC = c(0.701, 0.608, 0.692)) %>%
  pander()
```
...
A key finding of this analysis is that, according to our data source, injuries fell dramatically for nearly every team in 2016 compared to prior years, and injury levels have remained at this lower level since. We first interrograted our data source to ensure there weren't changes in its data collection methodology around this time. After finding no evidence that this was a data issue, we searched for a possible explanation for this fact. Doing so, we found that in response to public pressure the NFL made several rule changes around this time to prioritize player safety. This included 1) increasing the number of players in the "defenseless player category, 2) prohibiting ineffective helment types, and 3) coordinariting with coaches to ensure safe practices. We believe that these initiatives explain the drop in NFL injuries in 2016 and beyond.  

The majority of the our analysis looked at injuries simply by count, but we also looked at injuries by two different metrics: the number of games missed and the number of games injured. The number of games each season missed actually increased in the same year that the injury count decreased, indicating that perhaps injuries in more recent years have been more severe, or changes in NFL rules have forced players to miss more games that they would have otherwise played. We need to be specific in saying that the total count of injuries per player each season has declined, even though players have missed more games in recent years due to injury.
